...
for f, css in files:
    ...
    # check for CSS definition (with dot prefix and brace)
    if '.prod-price-sub{' in s or '.related-price-sub{' in s or '.feat-price-sub{' in s:
        print(f'{f}: already has CSS definition, skip')
        continue
    idx = s.find('</style>')
    if idx == -1:
