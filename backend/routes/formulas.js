...
router.post('/admin/personal-formulas', authMiddleware, requireModuleAccess('prescriptions'), (req, res) => {
  const { name, formulaType, items, usageInstructions, price } = req.body;
  ...
  db.prepare(`
    INSERT INTO personal_formulas (id, practitioner_id, practitioner_name, name, formula_type, items, usage_instructions, price)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `).run(id, req.admin.sub, req.admin.name, String(name).trim(), formulaType || 'granule', JSON.stringify(validItems), usageInstructions || '', Math.max(0, Number(price) || 0));
...
router.put('/admin/personal-formulas/:id', authMiddleware, requireModuleAccess('prescriptions'), (req, res) => {
  ...
  db.prepare(`
    ...
    SET name = ?, formula_type = ?, items = ?, usage_instructions = ?, price = ?, updated_at = CURRENT_TIMESTAMP
    WHERE id = ?
  `).run(
